...
// ── Design tokens ──────────────────────────────────────────────────────────
// The cockpit palette (see tailwind.config.js) expressed for this page's inline
// styles, so the marketing surface and the product read as one system.
//
// Two semantics invert when the ground goes dark:
//   · `navy` was both a dark background and the primary text colour. Backgrounds
//     are now `surface`/`raised`; `navy` keeps only its text role.
//   · "deep" shades get *lighter*, not darker — contrast runs the other way.
const C = {
  // Grounds
  bg: '#0a0b09',
  surface: '#121310',
  ...
  // Text ladder (t1 brightest → t4 faintest)
  navy: '#e8e8e3', // primary text
  t1: '#c9cac3',
  t2: '#9a9b93',
  t3: '#6b6d64',
  t4: '#5b5d55',

  // Mint — the brand accent
  teal: '#86c69a',
  tealDeep: '#a9d6b4', // lighter on dark: this is the "stronger" reading
  tealTint: '#1b2a20', // dark mint wash for chips/tiles
  tealBorder: '#3f6b50',
  onMint: '#0f1a12', // ink that sits on a mint fill

  // Gold — attention / review
  amber: '#e3a75c',
  amberTint: '#2e2312',
  amberText: '#f0c489',

  // Lavender — references / third category
  blue: '#b0a4d8',
  blueTint: '#241f33',
  ...
  // Text on the darkest panels
  onNavy: '#9a9b93',
  onNavy2: '#c9cac3',
  onNavy3: '#a9d6b4',
}

/** Mono stack — matches the cockpit's `--font-mono`. Numbers, labels, chips. */
...
// No `onNavy` variant any more: every ground on this page is dark, so the
// wordmark reads the same everywhere.
function Logo() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
      <span
        style={{
          width: 30, height: 30, borderRadius: 9, background: C.teal, color: C.onMint,
          fontWeight: 800, fontSize: 16, display: 'grid', placeItems: 'center',
        }}
      >
        e
      </span>
      <span style={{ fontWeight: 700, fontSize: 16, letterSpacing: '-0.3px', color: C.navy }}>
        eligo
      </span>
      <span
        style={{
...
function Btn({
  ...
  const styles: Record<string, CSSProperties> = {
    teal: { ...base, background: C.teal, color: C.onMint, fontSize: 14.5, padding: '14px 26px', boxShadow: '0 8px 24px rgba(134,198,154,0.22)' },
    // `dark` was the navy-filled prominent button on the light page. On a dark
    // ground the equivalent emphasis is a mint fill — it stays distinct from
    // `teal` by size, so nav/final CTAs keep a real primary action.
    dark: { ...base, background: C.teal, color: C.onMint, fontSize: 13, padding: '9px 17px', borderRadius: 10 },
    ghost: { ...base, background: 'transparent', color: C.navy, fontSize: 14.5, padding: '14px 24px', border: `1px solid ${C.btnBorder}` },
    ghostNavy: { ...base, background: 'transparent', color: C.navy, fontSize: 14.5, padding: '14px 24px', border: '1px solid rgba(255,255,255,0.28)' },
  }
...
function Eyebrow({ children, onNavy = false }: { children: ReactNode; onNavy?: boolean }) {
  return (
    // Mono uppercase eyebrows are the cockpit's section-label signature.
    <div style={{ fontFamily: MONO, fontSize: 11.5, fontWeight: 500, color: onNavy ? C.teal : C.tealDeep, textTransform: 'uppercase', letterSpacing: '1.4px' }}>
      {children}
    </div>
  )
}
...
function Banner({ L, onClose }: { L: Lfn; onClose: () => void }) {
  return (
    <div style={{ background: C.surface, color: C.onNavy3, fontSize: 12.5, fontWeight: 600, textAlign: 'center', padding: '9px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
      ...
      <a href="#" onClick={(e) => { e.preventDefault(); onClose() }} style={{ color: C.navy, fontWeight: 700, borderBottom: `1px solid ${C.teal}` }}>
...
function Nav({ L, lang, setLang, onEnterApp }: { L: Lfn; lang: Lang; setLang: (l: Lang) => void; onEnterApp: () => void }) {
  ...
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 40, background: 'rgba(10,11,9,0.85)', backdropFilter: 'blur(12px)', borderBottom: `1px solid ${C.line}` }}>
      ...
      {(['de', 'en'] as Lang[]).map((l) => (
        <button
          ...
          style={{
            ...
            background: lang === l ? C.raised : 'transparent',
...
function Hero({ L, onEnterApp }: { L: Lfn; onEnterApp: () => void }) {
  return (
    <section style={{ position: 'relative', background: 'linear-gradient(168deg, #101410 0%, #0d0e0b 55%, #12100c 100%)', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: -120, right: -120, width: 520, height: 520, borderRadius: '50%', background: 'radial-gradient(circle, rgba(134,198,154,0.10), transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ ...container, position: 'relative', padding: '74px 28px 92px' }}>
        <div className="lp-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 52, alignItems: 'center' }}>
          <div>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: C.tealTint, border: `1px solid ${C.tealBorder}`, borderRadius: 20, padding: '6px 13px', fontSize: 12, fontWeight: 700, color: C.tealDeep }}>
...
function HeroMock({ L }: { L: Lfn }) {
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ background: C.surface, border: `1px solid ${C.line2}`, borderRadius: 18, boxShadow: '0 26px 64px rgba(0,0,0,0.55)', overflow: 'hidden' }}>
        {/* dark header */}
        <div style={{ background: C.inset, padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
          <IconTile size={24} radius={7} bg={C.teal} color={C.onMint}><span style={{ fontSize: 12, fontWeight: 800 }}>e</span></IconTile>
          <span style={{ color: C.navy, fontSize: 12.5, fontWeight: 700 }}>Julia Schmidt</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: C.onNavy3, border: `1px solid ${C.tealBorder}`, borderRadius: 20, padding: '2px 8px' }}>94% Match</span>
      ...
      <div className="lp-floaty" style={{ position: 'absolute', left: -18, bottom: -22, background: C.raised, border: `1px solid ${C.line2}`, borderRadius: 14, boxShadow: '0 18px 44px rgba(0,0,0,0.6)', padding: '12px 15px', display: 'flex', alignItems: 'center', gap: 11 }}>
        <IconTile size={30} bg={C.tealTint} color={C.tealDeep}><FileText size={15} /></IconTile>
        <div>
          <div style={{ fontSize: 11, fontWeight: 800 }}>{L('Verifikations-Quittung', 'Verification receipt')}</div>
          <div style={{ fontFamily: MONO, fontSize: 11, color: C.t2 }}>sha256:77c1…be9a</div>
        </div>
      </div>
    </div>
  )
    ...
    <section style={{ background: C.surface }}>
      <div style={{ ...container, padding: '34px 28px' }}>
        <div className="lp-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }}>
          {stats.map(([v, l]) => (
            <div key={l}>
              <div style={{ fontSize: 38, fontWeight: 800, color: C.navy, letterSpacing: '-1.2px', fontFamily: MONO }}>{v}</div>
  ...
  return (
    <section style={{ ...container, padding: '88px 28px 20px' }}>
      ...
      <div className="lp-grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 18, marginTop: 44 }}>
        {cards.map((c) => (
          <div key={c.t} className="lp-agentcard" style={{ background: C.surface, border: `1px solid ${C.line2}`, borderRadius: 16, padding: 22 }}>
...
function ReceiptCard({ L }: { L: Lfn }) {
  ...
  return (
    <div style={{ background: C.raised, border: `1px solid ${C.line2}`, borderRadius: 14, padding: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
        <IconTile size={34} bg={C.tealTint} color={C.tealDeep}><FileText size={16} /></IconTile>
        <div>
      ...
      <div style={{ background: C.surf2, borderRadius: 12, padding: '12px 14px', display: 'grid', gap: 8 }}>
        {[
          ...
          <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            ...
            <span style={{ fontFamily: MONO, fontWeight: 700, color: C.navy }}>{v}</span>
          </div>
        ))}
      </div>
    </div>
...
function MatchingSplit({ L }: { L: Lfn }) {
  ...
  return (
    <section style={{ background: 'linear-gradient(180deg, #0e120f, #0a0b09)' }}>
      <div style={{ ...container, padding: '74px 28px' }}>
        <div className="lp-split" style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 56, alignItems: 'center' }}>
          <div className="lp-split-mock" style={{ background: C.surface, border: `1px solid ${C.line2}`, borderRadius: 18, boxShadow: '0 20px 50px rgba(0,0,0,0.45)', padding: 18 }}>
              ...
              {rows.map(([score, color, company, role, why]) => (
                <div key={company} style={{ display: 'flex', gap: 14, alignItems: 'center', border: `1px solid ${C.line}`, borderRadius: 12, padding: '12px 14px' }}>
                  <div style={{ width: 44, textAlign: 'center' }}>
                    <div style={{ fontSize: 20, fontWeight: 800, color, fontFamily: MONO }}>{score}</div>
                    <div style={{ fontSize: 8.5, fontWeight: 800, color: C.t3, letterSpacing: '0.5px', fontFamily: MONO }}>FIT</div>
...
function Compliance({ L }: { L: Lfn }) {
  ...
  return (
    <section style={{ ...container, padding: '74px 28px' }}>
      <div style={{ position: 'relative', background: C.surface, borderRadius: 22, padding: '44px 46px', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -100, right: -80, width: 380, height: 380, borderRadius: '50%', background: 'radial-gradient(circle, rgba(134,198,154,0.12), transparent 70%)' }} />
        <div className="lp-compliance-grid" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'center' }}>
          <div>
            ...
            <h2 className="lp-h2" style={{ fontSize: 32, fontWeight: 800, color: C.navy, letterSpacing: '-0.6px', margin: '12px 0 0' }}>
          ...
          <div className="lp-chip-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {chips.map((c) => (
              <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 8, background: C.panel, border: `1px solid ${C.panelBorder}`, borderRadius: 12, padding: '14px 15px' }}>
                ...
                <span style={{ fontSize: 12.5, fontWeight: 700, color: C.t1 }}>{c}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <section style={{ ...container, maxWidth: 920, padding: '40px 28px 80px', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginTop: 28 }}>
        <span style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#86c69a,#5f9c74)', color: C.onMint, fontWeight: 800, display: 'grid', placeItems: 'center' }}>MK</span>
...
function FinalCta({ L, onEnterApp }: { L: Lfn; onEnterApp: () => void }) {
  return (
    <section style={{ background: 'linear-gradient(168deg,#101410,#12100c)' }}>
...
function Footer({ L }: { L: Lfn }) {
  ...
  return (
    <footer style={{ background: C.surface, color: C.onNavy }}>
      <div style={{ ...container, padding: '52px 28px 34px' }}>
        <div className="lp-footer-grid" style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: 36 }}>
          ...
          {cols.map(([h, items]) => (
            <div key={h as string}>
              <div style={{ fontSize: 11, fontWeight: 800, color: C.navy, textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 14 }}>{h}</div>
